...
export function ClientesAdmin({ sessao, dias }: { sessao: SessaoTeeds | null; dias: number }) {
  ...
  return <section className="adm-clientes">
    ...
    {erro && <div className="ger-erro" role="alert">{erro}</div>}
    ...
    <div className="adm-filtros"><label>⌕<input aria-label="Buscar clientes por nome, e-mail, telefone ou CPF" placeholder="Buscar nome, e-mail, telefone ou CPF" value={busca} onChange={(e) => setBusca(e.target.value)} /></label><select aria-label="Filtrar clientes por status" value={filtro} onChange={(e) => setFiltro(e.target.value)}><option value="todos">Todos os status</option><option value="ativo">Ativos</option><option value="suspenso">Suspensos</option><option value="expirado">Expirados</option><option value="cancelado">Cancelados</option></select></div>
    <div className="ux-client-results"><span role="status" aria-live="polite">{buscando ? 'Atualizando resultados…' : `${n(lista.filtrados)} cliente${lista.filtrados === 1 ? '' : 's'} encontrado${lista.filtrados === 1 ? '' : 's'}`}</span>{(busca || filtro !== 'todos') && <button onClick={() => { setBusca(''); setFiltro('todos'); setPagina(0) }}>Limpar filtros</button>}</div>
