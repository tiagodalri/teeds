...
export function AdminPanel({ sessao, comissoes }: { sessao: SessaoTeeds; comissoes?: ReactNode }) {
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar"><div><span className="rot">Painel de controle</span><h2>Administração</h2><p>Gestão completa da {MARCA.prosa}</p></div><nav>{([['visao','Visão geral','⌂'],['inteligencia','Inteligência','◫'],['leads','Leads','◉'],['clientes','Clientes','◎'],['resultados','Resultados','±'],['movimentacoes','Depósitos e saques','⇅'],['acessos','Criar acesso','＋'],['aulas','Aulas','▶'],['planos','Planos','▣'],['produtos','Produtos','◇'],['comissoes','Comissões','↗']] as [Aba,string,string][]).map(([id,nome,ico]) => <button key={id} className={aba === id ? 'on' : ''} onClick={() => setAba(id)}><i>{ico}</i><span>{nome}</span>{id === 'clientes' && <b>{totalClientes}</b>}</button>)}</nav><small>Somente administradores podem visualizar e alterar estes dados.</small></aside>
