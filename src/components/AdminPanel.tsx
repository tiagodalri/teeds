...
import {
  atualizarAcessoCliente, criarAcessoCliente, definirProdutoCliente, listarClientes, listarComissoes, listarContasDeriv, listarMetricasRobos,
  listarPlanos, listarProdutos, listarProdutosClientes, salvarPlano, salvarProduto,
  type ClienteProdutoRegistro, type ClienteRegistro, type ComissaoDia, type ContaDerivRegistro, type MetricaRoboRegistro, type PlanoRegistro, type ProdutoRegistro,
...
export function AdminPanel({ sessao, comissoes }: { sessao: SessaoTeeds; comissoes?: ReactNode }) {
  ...
  const carregar = async () => {
    ...
    try {
      const [c, p, pr, l, co, m, mr] = await Promise.all([listarClientes(sessao), listarPlanos(sessao), listarProdutos(sessao), listarProdutosClientes(sessao), listarContasDeriv(sessao), listarComissoes(sessao, 90), listarMetricasRobos(sessao, 90)])
      setClientes(c); setPlanos(p); setProdutos(pr); setLiberacoes(l); setContas(co); setMovimento(m); setMetricasRobos(mr)
  ...
  // Só conta quem já entrou de verdade. A ficha nasce com "visto agora", então
  // sem este corte cada conta criada (as 11 mil importadas em 11/09, por
  // exemplo) apareceria como cliente ativo no dia do cadastro.
  const ativos24h = clientes.filter(c => c.totalAcessos > 0 && Date.now() - Date.parse(c.vistoEm) <= 864e5).length
  ...
  const ranking = clientes.map(c=>({cliente:c, comissao:clienteInfo(c.userId).movimento.filter(m=>!m.demo).reduce((s,m)=>s+m.comissao,0), operacoes:clienteInfo(c.userId).movimento.reduce((s,m)=>s+m.operacoes,0)})).sort((a,b)=>b.comissao-a.comissao)
  const planosDist = planos.map(p=>({nome:p.nome,total:clientes.filter(c=>c.planoId===p.id).length})).filter(p=>p.total)
  ...
  const paginacao = visiveis.length > POR_PAGINA && <div className="adm-paginas"><span>{inteiro(paginaAtual * POR_PAGINA + 1)}–{inteiro(Math.min(visiveis.length, (paginaAtual + 1) * POR_PAGINA))} de {inteiro(visiveis.length)} clientes</span><div><button onClick={() => setPagina(0)} disabled={paginaAtual === 0}>« Primeira</button><button onClick={() => setPagina(paginaAtual - 1)} disabled={paginaAtual === 0}>‹ Anterior</button><b>Página {inteiro(paginaAtual + 1)} de {inteiro(totalPaginas)}</b><button onClick={() => setPagina(paginaAtual + 1)} disabled={paginaAtual >= totalPaginas - 1}>Próxima ›</button><button onClick={() => setPagina(totalPaginas - 1)} disabled={paginaAtual >= totalPaginas - 1}>Última »</button></div></div>
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main"><header className="admin-top"><div><span className="rot">{MARCA.prosa} Admin</span><h1>{aba === 'visao' ? 'Visão geral' : aba === 'inteligencia' ? 'Inteligência do negócio' : aba === 'clientes' ? 'Gestão de clientes' : aba === 'resultados' ? 'Resultados por cliente' : aba === 'movimentacoes' ? 'Depósitos e saques' : aba === 'acessos' ? 'Criar novo acesso' : aba === 'planos' ? 'Planos e assinaturas' : aba === 'produtos' ? 'Catálogo de produtos' : 'Comissões e resultados'}</h1><p>{aba === 'visao' ? 'Tudo que precisa de atenção, em um só lugar.' : aba === 'inteligencia' ? 'Comportamento, carteira, atividade e receita organizados para decidir melhor.' : aba === 'clientes' ? 'Consulte, filtre e edite acessos existentes.' : aba === 'resultados' ? 'Quanto cada cliente operou, ganhou ou perdeu, e gerou de comissão — e o nosso número contra o da Deriv.' : aba === 'movimentacoes' ? 'Quanto entrou e quanto saiu das contas reais dos clientes na Deriv, dia a dia.' : aba === 'acessos' ? 'Cadastre uma pessoa e defina sua liberação inicial.' : aba === 'planos' ? 'Configure as opções comerciais da plataforma.' : aba === 'produtos' ? 'Gerencie os itens e extras do Marketplace.' : 'Acompanhe volume, operações e comissões da sua conta.'}</p></div><button className="admin-refresh" onClick={() => void carregar()} disabled={carregando}>↻ {carregando ? 'Atualizando…' : 'Atualizar'}</button></header>
      ...
      {aba === 'clientes' && <section className="admin-card full"><header><div><span className="rot">Base completa</span><h3>{clientes.length.toLocaleString('pt-BR')} clientes cadastrados</h3></div><button className="admin-primary" onClick={()=>setAba('acessos')}>＋ Novo cliente</button></header>{tabelaClientes}</section>}
