...
type Aba = 'visao' | 'clientes' | 'acessos' | 'planos' | 'produtos'
...
export function AdminPanel({ sessao }: { sessao: SessaoTeeds }) {
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar"><div><span className="rot">Painel de controle</span><h2>Administração</h2><p>Gestão completa da Teeds</p></div><nav>{([['visao','Visão geral','⌂'],['clientes','Clientes','◎'],['acessos','Criar acesso','＋'],['planos','Planos','▣'],['produtos','Produtos','◇']] as [Aba,string,string][]).map(([id,nome,ico]) => <button key={id} className={aba === id ? 'on' : ''} onClick={() => setAba(id)}><i>{ico}</i><span>{nome}</span>{id === 'clientes' && <b>{clientes.length}</b>}</button>)}</nav><small>Somente administradores podem visualizar e alterar estes dados.</small></aside>
    <main className="admin-main"><header className="admin-top"><div><span className="rot">Teeds Admin</span><h1>{aba === 'visao' ? 'Visão geral' : aba === 'clientes' ? 'Gestão de clientes' : aba === 'acessos' ? 'Criar novo acesso' : aba === 'planos' ? 'Planos e assinaturas' : 'Catálogo de produtos'}</h1><p>{aba === 'visao' ? 'Tudo que precisa de atenção, em um só lugar.' : aba === 'clientes' ? 'Consulte, filtre e edite acessos existentes.' : aba === 'acessos' ? 'Cadastre uma pessoa e defina sua liberação inicial.' : aba === 'planos' ? 'Configure as opções comerciais da plataforma.' : 'Gerencie os itens e extras do Marketplace.'}</p></div><button className="admin-refresh" onClick={() => void carregar()} disabled={carregando}>↻ {carregando ? 'Atualizando…' : 'Atualizar'}</button></header>
