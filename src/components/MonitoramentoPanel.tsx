...
function CabineEspelho({ s, estado, cliente, modo, saude, idade, rtt, desvio, eventos, aoFechar, passo, integridade }: {
  ...
  const parametros = [
    ...
    { rot: 'Recuperação', valor: s.config.fatorGale === 0 ? 'desligado' : temModos(s.roboId, s.config.parametros) ? `modo ${NOME_DO_MODO[modoDaConfig(s.roboId, s.config.fatorGale, s.config.lucroSobrePrejuizo, s.config)].toLowerCase()}` : `automática ${MARCA.prosa}` },
