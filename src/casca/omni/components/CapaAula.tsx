/**
 * Capas ilustradas das aulas — versao da OMNI.
 *
 * Copia da capa compartilhada (src/components/CapaAula.tsx) com duas trocas:
 * a paleta metalica deixa de ser ouro e passa a ser azul-aco, e o touro da
 * Teeds da lugar ao emblema de abertura da OMNI. Todo o resto — cenas,
 * materiais, composicao — e o mesmo desenho. Se a capa compartilhada mudar
 * de cena, esta copia precisa acompanhar; se mudar so a assinatura da marca,
 * nao.
 *
 * Ver src/casca/LEIA-ME.md.
 */
...
const OURO_CLARO = '#a9c6ec'  // azul-aco claro (o nome ficou pelo diff)
const OURO = '#6c93c6'
const OURO_ESCURO = '#1c3a66'
...
/** O emblema da OMNI: a abertura de seis laminas, em azul-aco. */
function Touro({ u, x, y, s, opacity = 1 }: { u: string; x: number; y: number; s: number; opacity?: number }) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`} opacity={opacity}>
      <polygon points="70.5,8.4 88.1,38.9 69.5,47.0 68.2,28.6" fill={`url(#ouro-${u})`} />
      <polygon points="87.6,41.9 70.0,72.4 53.7,60.4 69.0,50.0" fill={`url(#ouro-${u})`} />
      <polygon points="67.1,73.5 31.9,73.5 34.2,53.4 50.8,61.5" fill={`url(#ouro-${u})`} />
      <polygon points="29.5,71.6 11.9,41.1 30.5,33.0 31.8,51.4" fill={`url(#ouro-${u})`} />
      <polygon points="12.4,38.1 30.0,7.6 46.3,19.6 31.0,30.0" fill={`url(#ouro-${u})`} />
      <polygon points="32.9,6.5 68.1,6.5 65.8,26.6 49.2,18.5" fill={`url(#ouro-${u})`} />
      <circle cx="50.0" cy="40.0" r="8.8" fill="none" stroke={`url(#ouro-${u})`} strokeWidth="1.2" opacity="0.55" />
    </g>
  )
}
...
function Assinatura({ u }: { u: string }) {
  return (
    <g opacity="0.85">
      ...
      <text x="42" y="212" fontFamily="'Sora', 'Manrope', system-ui, sans-serif" fontSize="10.5" fontWeight="600"
        letterSpacing="5" fill={`url(#ouro-${u})`}>{MARCA.nome}</text>
    </g>
  )
}

/** Ficha metalica com relevo (azul-aco na OMNI). */
    ...
    case 'demo-vs-real':
      return (
        <g>
          ...
          <g filter={`url(#brilho-${u})`}>
            <text x="201" y="130" textAnchor="middle" fontFamily="'Sora', system-ui, sans-serif"
              fontSize="20" fontWeight="700" fill={cor}>vs</text>
          </g>
