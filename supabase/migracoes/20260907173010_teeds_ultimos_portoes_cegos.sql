-- Duas funções ainda perguntavam "é admin de alguma coisa?" em vez de
-- "é admin DESTA plataforma?". Elas já filtravam os dados pela marca, então
-- não vazavam número — mas um administrador só da OMNI conseguiria abrir o
-- relatório da Teeds e ver a estrutura vazia. Portão é portão.
...
as $function$
  ...
  where public.teeds_sou_admin_da(p_marca) and c.marca = p_marca
...
as $function$
  ...
  where (public.teeds_sou_admin_da(p_marca) or o.user_id = (select auth.uid()))
